...
import { 
    ...
    DEFAULT_CATEGORIES, DEFAULT_GRADES 
} from './constants';
...
export default function App() {
    ...
    const [userData, setUserData] = useState<UserData>(() => {
        try { return localStorage.getItem('userData_v2') ? JSON.parse(localStorage.getItem('userData_v2')!) : DEFAULT_USER_DATA; } catch { return DEFAULT_USER_DATA; }
    });
    ...
    const [settings, setSettings] = useState<AppSettings>(() => {
        try {
            ...
            return { ...DEFAULT_SETTINGS, ...parsed, islandLevels: parsed.islandLevels || DEFAULT_SETTINGS.islandLevels };
        ...
        triggerConfirm("確定從雲端 (Google 試算表) 讀取？這將覆蓋目前資料。", async () => {
            ...
            try {
                ...
                if (data && data.status === 'success' && data.data) {
                    ...
                    const newSettings = { ...loaded.settings, gasUrl: mergedGasUrl };
    ...
    const addExp = (amount: number) => {
        ...
        setUserData(prev => {
            ...
            return { ...prev, exp: newExp, logs: newLogs };
        });
    };

    const updateRow = (rowId: string | number, field: string, value: any) => {
        let expGain = 0; let dateField: string | null = null; let updates: any = {}; updates[field] = value;
        ...
        if (field === 'practice1' && value === true) expGain = settings.expPractice;
        if (field === 'correct1' && value === true) expGain = settings.expCorrect;
        if (field === 'note' && value === true) expGain = settings.expMemo;
        if (field.includes('score')) {
            if (value !== "") addExp(settings.expScoreEntry);
            const score = parseInt(value); if (!isNaN(score) && score >= settings.passingScore) expGain = settings.expPass;
        }
        if (field === 'score1') {
        ...
        if (expGain > 0) addExp(expGain); else setUserData(p => { const t = new Date().toISOString().split('T')[0]; return { ...p, logs: { ...p.logs, [t]: (p.logs[t] || 0) + 1 } }; });
    ...
    return (
        <div className="min-h-screen bg-[#FDFBF7] text-[#796E5B] font-sans p-4 md:p-8 selection:bg-[#8CD19D] selection:text-white overflow-x-hidden">
            ...
            <PomodoroModal show={showPomodoro} onClose={() => setShowPomodoro(false)} rows={allGlobalRows} settings={settings} addExp={addExp} triggerAlert={triggerAlert} triggerConfirm={triggerConfirm} onComplete={(taskId) => { if(!taskId) return; const row = allGlobalRows.find(r => r.id == taskId); if(row) updateRow(row.id, 'practice1', true); }} />
            <CalendarModal 
                show={showCalendar} 
            ...
            {/* ... (Settings and GasGuide remain same) ... */}
            {showSettings && (
                 <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#5E5244]/40 backdrop-blur-sm" onClick={() => setShowSettings(false)}>
                    <div className="bg-white rounded-3xl shadow-2xl p-6 w-full max-w-md border-4 border-[#F3F0E6] max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-[#FAF9F6] p-5 rounded-2xl border border-[#EFEBE0] shadow-inner"><label className="block text-sm font-bold text-[#9C9283] mb-4 border-b border-[#E5E7EB] pb-2">各項活動經驗值 (EXP) 設定</label><div className="grid grid-cols-2 gap-4"><div className="flex flex-col gap-1"><span className="text-xs font-bold text-[#9C9283] flex items-center gap-1"><div className="bg-[#8CD19D] p-0.5 rounded text-white"><CheckSquare size={10} strokeWidth={3} /></div> 筆記完成</span><input type="number" className="w-full p-2.5 text-center rounded-xl border border-[#D6CDB5] font-bold text-[#5E5244] focus:border-[#8CD19D] outline-none bg-white text-lg shadow-sm" value={settings.expMemo} onChange={(e) => setSettings({...settings, expMemo: parseInt(e.target.value) || 0})} /></div><div className="flex flex-col gap-1"><span className="text-xs font-bold text-[#9C9283] flex items-center gap-1"><Target size={12} className="text-[#F43F5E]" /> 刷題</span><input type="number" className="w-full p-2.5 text-center rounded-xl border border-[#D6CDB5] font-bold text-[#5E5244] focus:border-[#8CD19D] outline-none bg-white text-lg shadow-sm" value={settings.expPractice} onChange={(e) => setSettings({...settings, expPractice: parseInt(e.target.value) || 0})} /></div><div className="flex flex-col gap-1"><span className="text-xs font-bold text-[#9C9283] flex items-center gap-1"><Target size={12} className="text-[#F43F5E]" /> 訂正</span><input type="number" className="w-full p-2.5 text-center rounded-xl border border-[#D6CDB5] font-bold text-[#5E5244] focus:border-[#8CD19D] outline-none bg-white text-lg shadow-sm" value={settings.expCorrect} onChange={(e) => setSettings({...settings, expCorrect: parseInt(e.target.value) || 0})} /></div><div className="flex flex-col gap-1"><span className="text-xs font-bold text-[#9C9283] flex items-center gap-1">分數登記</span><input type="number" className="w-full p-2.5 text-center rounded-xl border border-[#D6CDB5] font-bold text-[#5E5244] focus:border-[#8CD19D] outline-none bg-white text-lg shadow-sm" value={settings.expScoreEntry} onChange={(e) => setSettings({...settings, expScoreEntry: parseInt(e.target.value) || 0})} /></div><div className="flex flex-col gap-1"><span className="text-xs font-bold text-[#9C9283] flex items-center gap-1"><Trophy size={12} className="text-[#D97706]" /> 分數通關</span><input type="number" className="w-full p-2.5 text-center rounded-xl border border-[#D6CDB5] font-bold text-[#5E5244] focus:border-[#8CD19D] outline-none bg-white text-lg shadow-sm" value={settings.expPass} onChange={(e) => setSettings({...settings, expPass: parseInt(e.target.value) || 0})} /></div><div className="flex flex-col gap-1"><span className="text-xs font-bold text-[#9C9283] flex items-center gap-1"><div className="text-[#F43F5E]">🍅</div> 番茄鐘完成</span><input type="number" className="w-full p-2.5 text-center rounded-xl border border-[#D6CDB5] font-bold text-[#5E5244] focus:border-[#8CD19D] outline-none bg-white text-lg shadow-sm" value={settings.expPomodoro} onChange={(e) => setSettings({...settings, expPomodoro: parseInt(e.target.value) || 0})} /></div></div></div>
