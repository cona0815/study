...
import { 
    ...
    FileText, Link as LinkIcon, Clock, Sparkles, Bot
} from 'lucide-react';
...
export const StudyTable: React.FC<Props> = ({ 
    activeSubject, settings, highlightedRowId, updateRow, deleteRow, addRow, openMemoModal, openAITutor
}) => {
                ...
                <table className="w-full text-left border-collapse min-w-[1000px] md:min-w-[1200px]">
                    ...
                    <tbody className="divide-y divide-[#F3F0E6]">
                        {activeSubject.rows.map((row, index) => {
                            ...
                            return (
                                <tr key={row.id} id={`row-${row.id}`} className={isHighlighted ? 'bg-[#FFF8E1] z-10 shadow-lg ring-2 ring-[#FCD34D] row-highlight' : 'group hover:bg-[#FAF9F6]'}>
                                    <td className="p-4 text-center font-bold text-[#D6D0C4] sticky left-0 z-10 bg-inherit border-r border-[#F3F0E6] group-hover:text-[#9C9283] transition-colors">{index + 1}</td>
