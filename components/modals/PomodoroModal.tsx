...
export const PomodoroModal: React.FC<Props> = ({ 
    show, onClose, rows, settings, onComplete, addExp, triggerAlert, triggerConfirm 
}) => {
    ...
    const handleAudioFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files.length > 0) {
            const newTracks = files.map(file => ({ name: file.name, url: URL.createObjectURL(file) }));
    ...
    const toggleAudio = () => {
        if (playlist.length === 0) { 
            triggerAlert("請點擊右上的圖示匯入 MP3 音樂檔案喔！"); 
            return; 
        }
    ...
    const stopAlarm = () => {
        ...
        const finish = (bonus = false) => {
            const amt = settings.expPomodoro * (bonus ? 2 : 1);
            addExp(amt);
            if (bonus && pomodoroTask) onComplete(pomodoroTask);
            else triggerAlert(`⏰ 專注完成！ +${amt} EXP`);
            resetTimer();
        };
                    ...
                    <div className="w-full bg-[#E5DCC5] rounded-3xl p-4 md:p-6 shadow-inner border border-[#D6CDB5] relative">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                <div onClick={() => audioInputRef.current?.click()} className="cursor-pointer hover:bg-black/10 p-1.5 rounded text-[#9C9283]"><Upload size={16} /></div>
                            </div>
                        </div>
                        
                        <div className="bg-[#374151] rounded-xl border-4 border-[#5E5244] p-4 h-24 md:h-32 flex flex-col items-center justify-center relative overflow-hidden mb-4 shadow-inner">
                            ...
                            <div className="text-white text-xs font-mono truncate w-full text-center bg-black/20 py-1 rounded">{playlist[currentTrackIndex]?.name || "NO TAPE INSERTED"}</div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 mb-4 w-full">
                            ...
                            <input type="range" min="0" max={duration || 0} value={currentTime} onChange={(e) => { if (audioRef.current) { audioRef.current.currentTime = parseFloat(e.target.value); setCurrentTime(parseFloat(e.target.value)); }}} className="flex-1 h-1.5 bg-[#D6CDB5] rounded-lg appearance-none cursor-pointer accent-[#8CD19D]" />
                        ...
                        <div className="flex justify-center gap-4">
                            <button onClick={() => { if(currentTrackIndex>0) setCurrentTrackIndex(p=>p-1) }} className="p-3 bg-[#E5E7EB] rounded-full shadow-md active:translate-y-0.5 transition-transform"><SkipBack size={20} /></button>
                            ...
                            <button onClick={() => { if(currentTrackIndex<playlist.length-1) setCurrentTrackIndex(p=>p+1) }} className="p-3 bg-[#E5E7EB] rounded-full shadow-md active:translate-y-0.5 transition-transform"><SkipForward size={20} /></button>
                        </div>
                        ...
                        {playlist.length > 0 && (
                            <div className="mt-4 bg-[#E5DCC5] rounded-lg p-2 max-h-32 overflow-y-auto custom-scrollbar border border-[#D6CDB5] text-xs w-full shadow-inner">
                                ...
                                {playlist.map((track, idx) => (
                                    <div key={idx} onClick={() => playTrack(idx)} className={`flex justify-between items-center p-1.5 rounded cursor-pointer mb-0.5 transition-colors ${idx === currentTrackIndex ? 'bg-[#5E5244] text-white shadow-sm' : 'text-[#5E5244] hover:bg-[#D6CDB5]'}`}>
                                        <span className="truncate w-32">{idx + 1}. {track.name}</span>
                        ...
                        <audio ref={audioRef} src={playlist[currentTrackIndex]?.url} onEnded={() => { if (currentTrackIndex < playlist.length - 1) setCurrentTrackIndex(p => p + 1); else { setCurrentTrackIndex(0); setIsPlaying(false); }}} onTimeUpdate={() => { if (audioRef.current) setCurrentTime(audioRef.current.currentTime); }} onLoadedMetadata={() => { if (audioRef.current) setDuration(audioRef.current.duration); }} className="hidden" />
